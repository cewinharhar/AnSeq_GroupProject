...
snaive und mean kann generell ausgeschlossen werden?

h=20?

box test

naive und rwf gleiches resultat?

Naive besser als drift methode? wieso?

## Fitting

```{r}
naive_shipment <- naive(df.Train, h=20)
autoplot(naive_shipment)
rwf_shipment <- rwf(df.Train, drif=TRUE, h=20)
autoplot(rwf_shipment)
mean_shipment <- meanf(df.Train, h=20)
autoplot(mean_shipment)

autoplot(rwf_shipment)+autolayer(fitted(rwf_shipment), series = "fitted")
summary(rwf_shipment)


checkresiduals(naive(df.Train))
checkresiduals(rwf(df.Train))
...
accuracy(mean_shipment, df.Test)
...
As can be seen from the above plots, the Damped Holt-Winter's multiplicative Method seems to be the most accurate (by eye) for the predictions. This doesnt Before evaluating the accuracy we want to check the residuals.
...
The Ljung-Box test shows a p-value \<\< 0.05 and therefore the NUll-Hypothesis is rejected. The residuals migt be auto-correlated.
...
@This: Da dis model inetue namens "df.lin"
...
accDf <- rbind(
  accuracy(df.lin$mean, df.Test),
  accuracy(df.hwAddDamped$mean, df.Test)
) 
rownames(accDf) <- c("linear", "hw damped Multiplicative")
